...
header('Access-Control-Allow_Methods: GET, PUT, POST, DELETE, OPTIONS');
...
$app->post('/Get_Data_Barang_Edit', 'Get_Barang_Edit');
...
{
    $data=$request->getPasedBody();
    $nama_barang=$data['nama_barang'];
    $qty=$data['qty'];
    $harga=$data['harga'];
    //$login=$data['login'];
    //$token=$data['token'];
    //$systemToken=apoToken($login);
    try {
        //if($systemToken == $token){
            $db=getDB();
            ...
            $db=null;
            if ($stmt)
            echo '{"Input_Barang": "input succsess"}';
            else
