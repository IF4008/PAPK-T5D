...
//request semua data yang berada pada tabel mahasiswa
...
//POST data mahasiswa untuk selanjutnya akan di simpan di tabel Mahasiswa
function Input_Mahasiswa($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "INSERT INTO mahasiswa(nama_mahasiswa, nim, semester) VALUES(:nama_mahasiswa ,:nim, :semester)";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("nama_mahasiswa", $nama_mahasiswa, PDO::PARAM_STR);
...
//request data yang berada pada tabel Mahasiswa berdasarkan id_mahasiswa
...
//POST data Mahasiswa ubah data berdasarkan id_mahasiswa
function Edit_Mahasiswa($request, $response)
{
    ...
    try {
        //if($systemToken == $token){
        $db = getDB();
        $sql = "UPDATE mahasiswa SET nama_mahasiswa=:nama_mahasiswa, nim=:nim, semester=:semester WHERE id_mahasiswa=:id_mahasiswa";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("id_mahasiswa", $id_mahasiswa, PDO::PARAM_STR);
        $stmt->bindParam("nama_mahasiswa", $nama_mahasiswa, PDO::PARAM_STR);
        $stmt->bindParam("nim", $nim, PDO::PARAM_STR);
...
//Untuk menghapus data mahasiswa berdasarkan id_mahasiswa
